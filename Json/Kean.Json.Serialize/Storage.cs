...
namespace Kean.Json.Serialize
{
		...
		Core.Serialize.Storage
	{
		...
		Core.Serialize.Data.Node Convert (Dom.Item item)
		{
			...
			if (item.NotNull ())
			{
				Dom.Attribute type = item.Attributes.Find (a => a.Name == "type");
				if (type.NotNull () && type.Value == "link")
					result = new Core.Serialize.Data.Link (this.GetTextContent (item)) {
						Name = item.Name,
						Region = item.Region
					};
				else
				if (item.All (n => !(n is Dom.Element)))
					result = new Core.Serialize.Data.String (this.GetTextContent (item)) {
						Name = item.Name,
						Region = item.Region
					};
				else
				{
					result = new Core.Serialize.Data.Branch () { Name = item.Name, Region = item.Region };
					foreach (Dom.Node node in item)
		...
		Dom.Element Convert (Core.Serialize.Data.Node element)
		{
			...
			if (element.NotNull ())
			{
				...
				else
				{
					...
					if (element is Core.Serialize.Data.Branch)
