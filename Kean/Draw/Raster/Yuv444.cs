...
namespace Kean.Draw.Raster
{
		...
		YuvPlanar
	{
		...
		{
			get { return new Color.Yuv((byte)(Color.Monochrome)this.Y[x, y], (byte)(Color.Monochrome)this.U[x, y], (byte)(Color.Monochrome)this.V[x, y]); }
			set
			{
				var yuv = value.Convert<Color.Yuv>();
				this.Y[x, y] = (Color.Monochrome)yuv.Y;
				this.U[x, y] = (Color.Monochrome)yuv.U;
				this.V[x, y] = (Color.Monochrome)yuv.V;
			}
		}
