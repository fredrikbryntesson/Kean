...
namespace Kean.Platform.Settings
{
		...
		Platform.Module
	{
		...
		protected internal override void Start()
		{
			try
			{
				Settings.Parser.Listen(this.root, Uri.Locator.FromPlatformPath(this.Application.ExecutablePath + "/" + System.IO.Path.GetFileNameWithoutExtension(this.Application.Executable).Replace(".vshost", "") + ".conf")).Dispose();
			}
			catch
			{
			}
			try
			{
				foreach (string file in System.IO.Directory.GetFiles(this.Application.ExecutablePath + "/Settings/", "*.conf", System.IO.SearchOption.AllDirectories))
					try
					{
						Settings.Parser.Listen(this.root, Uri.Locator.FromPlatformPath(file)).Dispose();
					}
					catch
					{
					} 
			...
			try
			{
				Settings.Parser.Listen(this.root, Uri.Locator.FromPlatformPath(this.Application.ExecutablePath + "/settings.conf")).Dispose();
			}
			catch
			{
			}
			try
			{
				foreach (Uri.Locator locator in this.configurations)
					try
					{
						Settings.Parser.Listen(this.root, locator).Dispose();
					}
					catch
					{
					}
