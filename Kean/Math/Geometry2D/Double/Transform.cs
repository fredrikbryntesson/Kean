...
namespace Kean.Math.Geometry2D.Double
{
		...
		IEquatable<Transform>
	{
        ...
        /// <returns></returns>
        public static Transform CameraTo2DTransform(Geometry3D.Double.EuclidTransform euclid, Math.Geometry2D.Double.Size fieldOfView, Math.Geometry2D.Double.Size windowWidth)
        {
            double k = Math.Double.Tangens(Math.Double.ToRadians(fieldOfView.Width / 2)) / (windowWidth.Width / 2);
            double translationX = euclid.Translation.Width;
            double translationY = euclid.Translation.Height;
            double relativeZ = 1 / (1 + euclid.Translation.Depth * k);
            double rotationX = euclid.Rotation.RotationX;
            double rotationY = euclid.Rotation.RotationY;
            double rotationZ = euclid.Rotation.RotationZ;

            double cosineRotZ = Math.Double.Cosine(Math.Double.ToRadians(rotationZ));
            double cosineRotX = Math.Double.Cosine(Math.Double.ToRadians(rotationX));
            double cosineRotY = Math.Double.Cosine(Math.Double.ToRadians(rotationY));
            double sineRotZ = Math.Double.Sine(Math.Double.ToRadians(rotationZ));
            double tangensRotY = Math.Double.Tangens(Math.Double.ToRadians(rotationY));
            double tangensRotX = Math.Double.Tangens(Math.Double.ToRadians(rotationX)); 

            double a = relativeZ * cosineRotZ / cosineRotX + translationX * k * tangensRotY / cosineRotX;
            double b = -relativeZ * (cosineRotZ * tangensRotY * tangensRotX + sineRotZ / cosineRotY) + translationX * k * tangensRotX;
            double c = relativeZ * sineRotZ / cosineRotX + translationY * k * tangensRotY / cosineRotX;
            double d = -relativeZ * (sineRotZ * tangensRotY * tangensRotX - cosineRotZ / cosineRotY) + translationY * k * tangensRotX;
            double t = translationX - relativeZ / k * (cosineRotZ * tangensRotY - sineRotZ * tangensRotX / cosineRotY);
            double u = translationY - relativeZ / k * (sineRotZ * tangensRotY + cosineRotZ * tangensRotX / cosineRotY);
            double p = k * tangensRotY / cosineRotX;
            double q = k * tangensRotX;
