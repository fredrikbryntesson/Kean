...
namespace Kean.Math.Geometry2D.Integer
{
		...
		IEquatable<Transform>
	{
        ...
        /// <returns></returns>
        public static Transform CreateProjection(int xRotation, int yRotation, int zRotation, Math.Geometry3D.Integer.Size translation, Size fieldOfView, Size windowSize)
        {
            int k = Math.Integer.Tangens(fieldOfView.Width / 2) / (windowSize.Width / 2);

            int tx = translation.Height;
            int ty = translation.Width;
            int tz = 1 / (1 + translation.Depth * k);

            int cosTau = Math.Integer.Cosine(zRotation);
            int cosTheta = Math.Integer.Cosine(xRotation);
            int cosPhi = Math.Integer.Cosine(yRotation);
            int sinTau = Math.Integer.Sine(zRotation);
            int tanPhi = Math.Integer.Tangens(yRotation);
            int tanTheta = Math.Integer.Tangens(xRotation);

            int a = tz * cosTau / cosTheta + tx * k * tanPhi / cosTheta;
            int b = -tz * (cosTau * tanPhi * tanTheta + sinTau / cosPhi) + tx * k * tanTheta;
            int c = tz * sinTau / cosTheta + ty * k * tanPhi / cosTheta;
            int d = -tz * (sinTau * tanPhi * tanTheta - cosTau / cosPhi) + ty * k * tanTheta;

            int t = tx - tz / k * (cosTau * tanPhi - sinTau * tanTheta / cosPhi);
            int u = ty - tz / k * (sinTau * tanPhi + cosTau * tanTheta / cosPhi);
            int p = k * tanPhi / cosTheta;
            int q = k * tanTheta;
