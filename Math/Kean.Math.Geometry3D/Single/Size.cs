...
namespace Kean.Math.Geometry3D.Single
{
...

    {
		...
		#endregion
		#region properties
		public float Norm { get { return Math.Single.SquareRoot((Math.Single.Squared(this.Width) + Math.Single.Squared(this.Height) + Math.Single.Squared(this.Depth))); } }
		public float Azimuth { get { return Math.Single.ArcusTangensExtended(this.Width, this.Height); } }
		...
		{
			return string.Format(format, ((float)this.Width).ToString(), ((float)this.Height).ToString(), ((float)this.Depth).ToString());
		}
		#endregion
        #region Casts
        ...
        {
            return new Size((float)value.Width, (float)value.Height, (float)value.Depth);
        }
