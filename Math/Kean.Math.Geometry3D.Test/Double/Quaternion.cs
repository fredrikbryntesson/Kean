...
namespace Kean.Math.Geometry3D.Test.Double
{
		...
		Kean.Test.Fixture<Quaternion>
	{
		...
		public void LogarithmExponential()
		{
			...
			Verify(quaternion.Exponential().Logarithm().Imaginary.Distance(quaternion.Imaginary), Is.EqualTo(0).Within(this.Precision));
			Verify(quaternion.Logarithm().Exponential().Imaginary.Distance(quaternion.Imaginary), Is.EqualTo(0).Within(this.Precision));
		}
		[Test]
		public void Norm()
		{
		...
		{
			Target.Point direction = new Target.Point(1, 1, 1);
			Target.Quaternion quaternion = Target.Quaternion.CreateRotation(Kean.Math.Double.ToRadians(120), direction);
			Target.Point point = new Target.Point(5, 6, 7);
			Target.Point point2 = new Target.Point(7, 5, 6);
			Verify((quaternion * point).Distance(point2), Is.EqualTo(0).Within(this.Precision));
		}
		[Test]
		public void Casting()
		{
		...
		public void RotationDirectionRepresentation1()
		{
			...
			Verify(q.Direction.Distance(direction), Is.EqualTo(0).Within(this.Precision));
		}
		[Test]
		...
		public void Action2()
		{
			...
			Kean.Math.Matrix.Double rotatedPoint = rotation * (Kean.Math.Matrix.Double)(new double[] { this.p0.X, this.p0.Y, this.p0.Z });
			double[] rotatedPointArray = (double[])rotatedPoint;
			Target.Point rotatedPointRotation = new Target.Point(rotatedPointArray[0], rotatedPointArray[1], rotatedPointArray[2]);
			Verify(rotatedPointQuaternion.Distance(rotatedPointRotation), Is.EqualTo(0).Within(this.Precision));
			Verify(quaternion.RotationX, Is.EqualTo(roll).Within(this.Precision));
			Verify(quaternion.RotationY, Is.EqualTo(pitch).Within(this.Precision));
			Verify(quaternion.RotationZ, Is.EqualTo(yaw).Within(this.Precision));
		}
