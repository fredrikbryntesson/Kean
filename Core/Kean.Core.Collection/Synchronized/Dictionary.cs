...
namespace Kean.Core.Collection.Synchronized
{
        ...
        where TKey : IEquatable<TKey>
	{
		...
		{
			get { lock (this.Lock) return this.data[key]; }
			set { lock (this.Lock) this.data[key] = value; }
		}
		public bool Contains(TKey key)
		{
			lock (this.Lock) return this.data.Contains(key); ;
		}
		public bool Remove(TKey key)
		{
			lock (this.Lock) return this.data.Remove(key); ;
		}
		#endregion
		...
		{
			lock (this.Lock) return this.data.Equals(other); ;
		}
		#endregion
		...
		{
			lock (this.Lock) return base.GetHashCode();
		}
		public override string ToString()
		{
			lock (this.Lock) return base.ToString();
		}
		#endregion
	}
}
